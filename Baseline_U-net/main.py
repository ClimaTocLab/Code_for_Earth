...
if TRAIN:
    ...
    # --- Training ---
    if fast_debug: epochs=1 
    else: epochs=200
    train(model, train_loader, val_loader, epochs=epochs, device=device)
    
    # --- Save stats ---
    with open("Baseline_U-net/outputs/y_mean-y_std.pkl", "wb") as f:
        pickle.dump({"y_mean": y_mean, "y_std": y_std, "n_channels_in": X_train.shape[1]}, f)
...
if TEST:
    ...
    # --- Model setup ---
    model = UNetSuperRes(n_channels_in=n_channels_in)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Load trained model weights
    try: model.load_state_dict(torch.load(path))  # GPU default
    except: model.load_state_dict(torch.load(path, map_location=torch.device('cpu')))
